...
public class DepositOrderService extends AbstractEntityService<DepositOrder, String, DepositOrderRepository> {
    ...
    public List<DepositOrder> findFinishedOrder() {
        return repository.findByStatusOrderByDateFinishedDesc(OrderStatusEnum.FINISHED);
    }
    ...
    @Override
    public DepositOrder create(DepositOrder entity) {
        entity.setId(IDGenerator.order());
        entity.setTimeStart(new Date());
        entity.setTimeEnd(new Date(System.currentTimeMillis() + 1000 * 60 * 5L));
