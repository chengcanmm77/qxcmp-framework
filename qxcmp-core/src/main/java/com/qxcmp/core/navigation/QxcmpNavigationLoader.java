...
public class QxcmpNavigationLoader implements QxcmpInitializer {
    ...
    @Override
    public void init() {
        applicationContext.getBeansOfType(NavigationLoader.class).values().stream().sorted(new AnnotationAwareOrderComparator()).forEach(navigationConfigurator -> {
            log.info("--- loading navigation {}", navigationConfigurator.getClass().getSimpleName());
            navigationConfigurator.configNavigation(navigationService);
        });
    }
}
