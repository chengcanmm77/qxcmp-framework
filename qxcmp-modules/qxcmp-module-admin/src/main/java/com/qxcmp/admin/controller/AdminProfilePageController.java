...
public class AdminProfilePageController extends QxcmpController {
    ...
    public ModelAndView infoPage(@Valid final AdminProfileInfoForm form, BindingResult bindingResult) {
        ...
        }, (stringObjectMap, overview) -> overview.addLink("返回", QXCMP_ADMIN_URL + "/profile/info"));
    }
    ...
    public ModelAndView securityPage() {
        ...
        return page().addComponent(new Segment()
                ...
                .addComponent(new Table().setBasic().setHeader((AbstractTableHeader) new TableHeader()
                        .addRow(new TableRow().addCell(new TableHead("登录密码").setAlignment(Alignment.CENTER)).addCell(new TableData("安全性高的密码可以使帐号更安全")).addCell(new TableData(StringUtils.isNotBlank(user.getPassword()) ? new Icon("check circle").setColor(Color.GREEN) : new Icon("warning circle").setColor(Color.ORANGE)).setAlignment(Alignment.CENTER)).addCell(new TableData(new Button("修改", QXCMP_ADMIN_URL + "/profile/security/password").setBasic()).setAlignment(Alignment.CENTER)))
                        .addRow(new TableRow().addCell(new TableHead("支付密码").setAlignment(Alignment.CENTER)).addCell(new TableData("支付密码可以提高在支付场合的安全性")).addCell(new TableData(StringUtils.isNotBlank(user.getPayPassword()) ? new Icon("check circle").setColor(Color.GREEN) : new Icon("warning circle").setColor(Color.ORANGE)).setAlignment(Alignment.CENTER)).addCell(new TableData(new Button(StringUtils.isNotBlank(user.getPayPassword()) ? "修改" : "设置", QXCMP_ADMIN_URL + (StringUtils.isNotBlank(user.getPayPassword()) ? "/profile/security/pay/password/edit" : "/profile/security/pay/password")).setBasic()).setAlignment(Alignment.CENTER)))
                        .addRow(new TableRow().addCell(new TableHead("手机绑定").setAlignment(Alignment.CENTER)).addCell(new TableData("手机号可以直接用于登录、找回密码等")).addCell(new TableData(StringUtils.isNotBlank(user.getPhone()) ? new Icon("check circle").setColor(Color.GREEN) : new Icon("warning circle").setColor(Color.ORANGE)).setAlignment(Alignment.CENTER)).addCell(new TableData(new Button("绑定", QXCMP_ADMIN_URL + "/profile/security/phone").setBasic()).setAlignment(Alignment.CENTER)))
                        .addRow(new TableRow().addCell(new TableHead("邮箱绑定").setAlignment(Alignment.CENTER)).addCell(new TableData("邮箱可以直接用于登录、找回密码等")).addCell(new TableData(StringUtils.isNotBlank(user.getEmail()) ? new Icon("check circle").setColor(Color.GREEN) : new Icon("warning circle").setColor(Color.ORANGE)).setAlignment(Alignment.CENTER)).addCell(new TableData(new Button("绑定", QXCMP_ADMIN_URL + "/profile/security/email").setBasic()).setAlignment(Alignment.CENTER)))
                        .addRow(new TableRow().addCell(new TableHead("密保问题").setAlignment(Alignment.CENTER)).addCell(new TableData("建议设置三个容易记住，且最不容易被他人获取的问题及答案，更有效保障您的密码安全")).addCell(new TableData(hasSecurityQuestion ? new Icon("check circle").setColor(Color.GREEN) : new Icon("warning circle").setColor(Color.ORANGE)).setAlignment(Alignment.CENTER)).addCell(new TableData(new Button("设置", QXCMP_ADMIN_URL + "/profile/security/question").setBasic()).setAlignment(Alignment.CENTER)))
                )))
    ...
    public ModelAndView securityPasswordPage(@Valid final AdminProfileSecurityPasswordForm form, BindingResult bindingResult) {
        ...
        }, (stringObjectMap, overview) -> overview.addLink("返回", QXCMP_ADMIN_URL + "/profile/security"));
    }
    ...
    public ModelAndView securityEmailPage(@Valid final AdminProfileSecurityEmailForm form, BindingResult bindingResult, final AdminProfileSecurityEmailBindForm bindForm) {
        ...
        try {
            ...
            request.getSession().setAttribute(ProfilePageHelper.EMAIL_BINDING_SESSION_ATTR, code);
            request.getSession().setAttribute(ProfilePageHelper.EMAIL_BINDING_CONTENT_SESSION_ATTR, form.getEmail());
            ...
            return page(new Overview(new IconHeader("邮箱绑定失败", new Icon("warning circle"))).addComponent(new P(e.getMessage())).addLink("返回安全设置", QXCMP_ADMIN_URL + "/profile/security")).build();
        }
    }
    ...
    public ModelAndView securityEmailBindPage(@Valid final AdminProfileSecurityEmailBindForm form, BindingResult bindingResult) {
        ...
        if (Objects.isNull(request.getSession().getAttribute(ProfilePageHelper.EMAIL_BINDING_SESSION_ATTR)) || !StringUtils.equals(form.getCaptcha(), (String) request.getSession().getAttribute(ProfilePageHelper.EMAIL_BINDING_SESSION_ATTR)) || bindingResult.hasErrors()) {
            return page(new Overview(new IconHeader("邮箱绑定失败", new Icon("warning circle"))).addComponent(new P("绑定验证不正确或者已过期")).addLink("返回安全设置", QXCMP_ADMIN_URL + "/profile/security")).build();
        }

        return submitForm(form, context -> {
            try {
                User user = currentUser().orElseThrow(RuntimeException::new);
                String email = (String) request.getSession().getAttribute(ProfilePageHelper.EMAIL_BINDING_CONTENT_SESSION_ATTR);
    ...
    public ModelAndView securityQuestionPage(final AdminProfileSecurityQuestionForm form) {
        return page().addComponent(new TextContainer().addComponent(new Segment().addComponent(convertToForm(form))))
                ...
                .addObject("selection_items_question1", ProfilePageHelper.QUESTIONS_LIST_1)
                .addObject("selection_items_question2", ProfilePageHelper.QUESTIONS_LIST_2)
                .addObject("selection_items_question3", ProfilePageHelper.QUESTIONS_LIST_3)
                .build();
    }
